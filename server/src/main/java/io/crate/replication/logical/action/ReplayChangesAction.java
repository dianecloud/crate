    ...
    public static class TransportAction extends TransportWriteAction<Request, Request, ReplicationResponse> {
        ...
        void performOnPrimary(IndexShard primary,
            ...
            for (int i = offset; i < request.changes().size(); i++) {
                ...
                Long currentTableVersion = getTableVersion(request);
                try {
        ...
        private Long getTableVersion(Request request) {
            return getTableVersion(clusterService.state(), request.shardId().getIndexUUID());
        }
        ...
        private static boolean isTableVersionUpdated(ClusterState cs, String indexUUID, Long baseVersion) {
            Long currentTableVersion = getTableVersion(cs, indexUUID);
            if (currentTableVersion == null) {
                return false;
            }
            return currentTableVersion > baseVersion;
        }
